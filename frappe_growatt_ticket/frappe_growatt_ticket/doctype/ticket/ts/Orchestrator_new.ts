...
//   requiredWorkflow: string;                              // Estado necessário do doctype anterior
...
//   afterCreation?: (form: FrappeForm) => Promise<Record<string, any>>;  // Função para preparar dados antes da criação
//   validation?: (form: FrappeForm) => Promise<{ isValid: boolean; errorMessage?: string }>;  // Validação customizada adicional
// }> = {
//   "Initial Analysis": {
//     doctype: "Initial Analysis",
//     requiredWorkflow: "Finished",
//     dependencies: [],
...
//       // Verifica se o solution_select do Initial Analysis é "Deep Analysis"
//       const solution_select = await agt.utils.get_value_from_any_doc(form, 'Initial Analysis', 'ticket_docname', 'solution_select');

//       if (solution_select !== "Deep Analysis") {
//         return {
//           isValid: false,
//           errorMessage: "Cannot create Checklist: Initial Analysis solution_select must be 'Deep Analysis'."
//         };
//       }

//       return { isValid: true };
//     },
//     afterCreation: async (form) => {
...
//         workflow_state: depConfig.requiredWorkflow
//       },
//       fields: ['name'],
//     });

//     if (existingDocs && existingDocs.length > 0) {
//       // Dependência satisfeita, agora verifica a validação customizada se existir
//       if (config.validation) {
//         const customValidation = await config.validation(form);
//         if (!customValidation.isValid) {
//           return {
//             isValid: false,
//             errorMessage: customValidation.errorMessage || `Custom validation failed for ${resolveDoctypeName(form, subWorkflowKey)}.`
//           };
//         }
//       }
//       return { isValid: true };
...
//     if (config.validation) {
//       const validation = await config.validation(form);
//       if (!validation.isValid) {
//         console.warn(validation.errorMessage || `Custom validation failed for ${doctypeName}.`);
//         return null;
//       }
//     }
...
//     // Prepara dados adicionais se houver função afterCreation configurada
//     let additionalData: Record<string, any> = { ticket_docname: "docname" };
//     if (config.afterCreation) {
//       const preparedData = await config.afterCreation(form);
...
//       }
//     }
//     // Tenta criar o Checklist se não existir e se as validações passarem
//     if (sub_workflow_value !== "Checklist") {
//       const validation = await validateCreationFlow(form, "Checklist");
//       if (validation.isValid) {
//         // console.log("✅ Validação passou para Checklist - tentando criar...");
//         await handleSubWorkflowStep(form, "Checklist");
//       // } else {
//       //   console.log("❌ Validação falhou para Checklist:", validation.errorMessage);
//       }
//     }
...
//     //   }
//     // }
//     if (sub_workflow_value !== "Compliance Statement" && (sub_workflow_value === "Proposed Dispatch")) {
